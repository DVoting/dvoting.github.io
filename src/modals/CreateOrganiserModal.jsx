...
const CreateOrganiserModal = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (res.error) {
      ...
      toast.error(
        Message(
          `error ${res.status}`,
          `${(res.data && res.data.message) || res.statusText}`
        ),
        {
    ...
    setInputs((prev) => ({ ...prev, organisationName: "" }));
    setShow(false);
    setRedirect("/org");
  };
  ...
  const validInputs = () => {
    if (inputs.organisationName.length < 4) {
      toast.error(
        Message("Organiser Name", "must be at least 4 characters long"),
        {
          position: toast.POSITION.BOTTOM_RIGHT,
        }
      );
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id='createOrganiserModal'>Create Organiser</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
